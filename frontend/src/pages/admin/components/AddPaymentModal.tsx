...
const AddPaymentModal: React.FC<AddPaymentModalProps> = ({ order, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const shopKeeperId = typeof order.shopKeeper === 'object' ? order.shopKeeper?._id : order.shopKeeper;
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      {/* Modal */}
      {/* Modal */}
      <div className="relative w-full max-w-md bg-navy-mid border border-navy-light rounded-2xl shadow-2xl overflow-hidden animate-fadeIn max-h-[90vh] flex flex-col">
          ...
          <form onSubmit={handleSubmit} className="px-6 pb-6 pt-4 space-y-4">
            ...
            <div className="flex gap-3 pt-2">
              <button
                type="button"
                onClick={onClose}
                className="flex-1 border border-navy-light text-gray-300 hover:text-white hover:border-gray-500 py-3 rounded-xl font-bold text-[12px] uppercase tracking-widest transition-all cursor-pointer"
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 bg-electric text-white py-3 rounded-xl font-bold text-[12px] uppercase tracking-widest transition-all hover:bg-blue-600 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2 cursor-pointer"
              >
                {isLoading ? (
